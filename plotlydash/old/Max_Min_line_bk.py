...
def create_dash(flask_app):
        ...
        def maxminline_strategy(n_intervals, data):
                if n_intervals == 0:
                    ...
                    slopes10 = df['MA10'].rolling(10).apply(lambda s: linregress(s.reset_index())[0])
                    ...
                    df['min_close_idx'] = np.nan
                    
                    idx_max = df.resample('20min', on='Datetime')["Close"].agg(lambda x: np.nan if((x.isna().all()) or (df['min_close_idx'].notna().any())) else x.idxmax())
