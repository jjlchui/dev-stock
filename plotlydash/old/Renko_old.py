...
def create_dash(flask_app):
        ...
        def update_df(n_intervals):
                if n_intervals == 0:
                    ...
                    filename = "2022-10-27 NQ=F USTime_feather_stock_data.feather"
                    cwd = os.getcwd()
        ...
        def maxminline_strategy(n_intervals, data):
                if n_intervals == 0:
                    ...
                    T_Sell[11] = df.Close.iloc[11]
                    """
                    def compare(T_Buy, T_Sell):
                        if (len(T_Buy) > len(T_Sell)):
                        ### check price go dn after buy
                            lst_tbuy = [i for i in T_Buy if not math.isnan(i)][-1]
                            if (df['Close'].iloc[-1] < (lst_tbuy + stop_loss_limit)):
                                T_Sell_lc = df['Close'].iloc[-1]

                        else:
                        ### check price go up after short
                            lst_tsell = [i for i in T_Sell if not math.isnan(i)][-1]
                            if (df['Close'].iloc[-1] > (lst_tsell + stop_loss_limit)):
                                T_Buy_lc = df['Close'].iloc[-1]
                    ...
                    if((len(T_Buy) > len(T_Sell)) & (df.Close.iloc[-1] < (lst_tbuy + stop_loss_limit))):
                        T_Sell[-1] = df.Close.iloc[-1]
                    elif((len(T_Sell) > len(T_Buy)) & (df.Close.iloc[-1] > (lst_tsell - stop_loss_limit))):
                        T_Buy[-1] = df.Close.iloc[-1]
                    
                    
                    T_Sell = df.Close.apply(lambda x: x if (x < (lst_tbuy + stop_loss_limit)) else T_Sell)
                    ...
                    ###price go down 
                    df['Buy_n'] = np.where(T_Buy == "NaN", np.nan, T_Buy)
                    df['Buy_loss_limit'] = df['Buy_n'].astype(float) - stop_loss_limit 
                    df['Buy_loss_limit'] = df['Buy_loss_limit'].ffill()
                    
                    ###price go up 
                    df['Sell_n'] = np.where(T_Sell == "NaN", np.nan, T_Sell)
                    df['Sell_loss_limit'] = df['Sell_n'].astype(float) + stop_loss_limit 
                    df['Sell_loss_limit'] = df['Sell_loss_limit'].ffill()
                    """
                    if (df['Buy_n'].count() > df['Sell_n'].count()):
                        lst_sell_loss_limit = [i for i in df['Sell_loss_limit'] if not math.isnan(i)][-1]
                        Sell_n_f  = [i for i in df['Close'] if i > lst_sell_loss_limit][0]
                        
                        df['Sell_n_f'] = np.where(
                                             (df['Close'] == Sell_n_f) 
                                            , df.Close, T_Sell)
                    ...
                    df['Sell_n_f'] = np.where((df['Buy_n'].count() > df['Sell_n'].count()) & 
                                         ...
                                         #((df['Buy_loss_limit'].astype(float) + 2.00) < (df['Close'].astype(float))))
                                        , df.Close, T_Sell)
                    ...
                    df['Buy_n_f'] = np.where((df['Sell_n'].count() > df['Buy_n'].count()) &
                                        ...
                                        #((df['Sell_loss_limit'].astype(float) + 2.00) > (df['Close'].astype(float))))
                                        , df.Close, T_Buy)
                    T_Buy = df['Buy_n_f']       
