...
def ma_strategy(data):
    ...
    slopes = [i*0 for i in range(n-1)]
    for i in range(n, len(df.Close)+1):
        y=df.Close[i-n:i]
        x=np.array(range(n))
        y_scales = (y-y.min())/(y.max()-y.min())
        x_scales = (x-x.min())/(x.max()-x.min())
...
def macd_strategy(data):
    ...
    """
    df['slope_up_upper'] = np.where((df['MACD_12_26_9'] > 0) & (df['MACDs_12_26_9'] > 0) &
                              (macd_max_40 < macd_max_20), "y", "n")
    
    df['slope_up_lower'] = np.where((df['MACD_12_26_9'] < 0) & (df['MACDs_12_26_9'] < 0) & 
                                   (macd_max_40 < macd_max_20), "y", "n")
    
    df['slope_dn_upper'] = np.where((df['MACD_12_26_9'] > 0) & (df['MACDs_12_26_9'] > 0) &
                              (macd_max_40 > macd_max_20), "y", "n")
    
    df['slope_dn_lower'] = np.where((df['MACD_12_26_9'] < 0) & (df['MACDs_12_26_9'] < 0) & 
                                   (macd_max_40 > macd_max_20), "y", "n")
    """
...
def create_dash(flask_app):
        ...
        app=dash.Dash(server=flask_app, name="stock_dash", url_base_pathname=("/json/"), prevent_initial_callbacks=True)
        ...
        def update_graph(n_intervals):
            ...
            if n_intervals == 0:
                ...
                file = 'D://Development//2022-06-17 NQ=Fout_stock_data.csv'
                """
                pwd = os.getcwd()
                os.chdir(os.path.dirname(file))
                """
