...
def macd_strategy(data):
    ...
    #old_slope20 = slope20(df.Close)
    #old_ma10_50 = slope20(df.MA50)
...
def create_dash(flask_app):
            ...
            if n_intervals == 0:
                ...
                figure = go.Figure(
                    data = [go.Scatter(x=df.index, y=df.Close, line=dict(color='#fc0080', width=2), 
                            ...
                            go.Scatter(x=df.index, y=df.MA10, line=dict(color="#fad51e", width=1.5), 
                                name = 'MA10',
                                hoverinfo = 'text',
                                hovertext =
                                '<b>Time</b>: ' + df.Datetime.astype(str) + '<br>' +
                ...
                go.Scatter(x=df.index, y=buymacd, name="macdup", mode="markers",
                              marker=dict(
                              symbol="5" ,
                              color="lightskyblue",
                              size=12)), 

                go.Scatter(x=df.index, y=sellmacd, name="macddn", mode="markers",
                              marker=dict(
                              symbol="6" ,
                              color="MediumPurple",
                              size=12)),
        ...
        def profit(n_intervals,ave_bb, ave_ss):
            ...
            if n_intervals == 0:
                ...
                if ave_ss is None  and ave_bb is None:
                    ave_ss = 0
                    ave_bb = 0
                    profit = 0
                elif ave_ss is None:
