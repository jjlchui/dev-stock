...
def maxminline_strategy(data):
    ...
    Buy = np.where((df['Close'] == df['min_close_idx'] ) &
                    ...
                    ,df['Close'], np.nan)
    Buylist = np.where((Buy==df['Close']), 1, 0)
    ...
    Selllist = np.where((Sell==df['Close']), -1, 0)
    ...
    """
    ### call count ###
    BS_list = Buylist + Selllist
    df['a']= BS_list
    
    fv_index= df['a'].ne(0).idxmax()
    #fv_index = df.loc[frist_value]
    
    df['as1']=df['a'].iloc[0:fv_index]
    df['as1'] = df['as1'].fillna(0)
    df['as1'] = df['as1'].astype(int)
    b = bs_count(df,df['a'], df['as1'],fv_index)
