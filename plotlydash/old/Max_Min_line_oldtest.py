...
def maxminline_strategy(data):
    ...
    return Buy, Sell, tot_buy.iloc[-1], tot_sell.iloc[-1], Buy_count.iloc[-1], Sell_count.iloc[-1]
...
def create_dash(flask_app):
                    ...
                    html.Div([
                        html.Div([
                            ...
                            animation_options = dict(
                                ...
                                figure=go.Scatter(x=[], y=[], line=dict(color='#fc0080', width=2)),
        ...
        def update_graph(n_intervals, data, existing):
            ...
            if n_intervals == 0:
                ...
                tt_buy = tot_buy
                tt_sell =  tot_sell
                bbuy_count =  buy_count
                ssell_count =  sell_count
        ...
        def append_data(n_intervals, existing):
                if n_intervals == 0:
                    raise PreventUpdate
                else:
        ...
        @cc.callback(
                ...
                [Input('update_value', 'n_intervals'),
                 Input('df_value', 'data')],
                [State('price_rsi', 'figure')])

        def update_rsi(n_intervals, data, existing):
            if n_intervals == 0:
                raise PreventUpdate
            else:
