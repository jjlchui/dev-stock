...
def create_dash(flask_app):
        ...
        df = pd.read_csv('D:\\Development\\flask dev\\stock\\data\\2022-09-28 NQ=F USTime_out_stock_data.csv')
        ...
        app.layout = html.Div([
            ...
            ##### Store data
                ...
                dcc.Store(
                    id='candle_data', data=dataS,
                ),
                dcc.Store(id='buy'),
                dcc.Store(id='sell'),
                dcc.Store(id='buy_nplist'),
                dcc.Store(id='sell_nplist'),
                dcc.Store(id='buy_tcount',storage_type='local'),
                dcc.Store(id='sell_tcount',storage_type='local'),
                dcc.Store(id='tot_tbuy',storage_type='local'),
                dcc.Store(id='tot_tsell',storage_type='local'),
            ...
            ##### Graph Display
                html.Div([
                    html.Div([
                        ...
                        html.Div([
                            html.Title('test_layout', 
                            style = {'color': 'black'},
                            ),

                            dcc.Graph(id = 'test_layout', figure=dict(data=dataS,layout=layoutS),
                                      animate=False,
                                    style={'width': 'auto',
                                           'height': '60vh',
                                           'border': '1px #5c5c5c solid',},
        ...
        def update_df(n_intervals):
                if n_intervals == 0:
                    ...
                    df = pd.read_csv(file, iterator=True, chunksize=2000)
        ...
        app.clientside_callback(
            ...
            """,
            Output('test_layout', 'figure'),
            Input('clientside-interval','n_intervals'),
            Input('candle_data', 'data'),
        ...
        @cc.callback(
                    ...
                    [Input('update_value', 'n_intervals'),
                     State('tot_tbuy', 'data'),
                     State('tot_tsell', 'data'),
                     State('buy_tcount', 'data'),
                     State('sell_tcount', 'data'),])
