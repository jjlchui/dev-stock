...
def get_stock_data():
    ...
    if (Running == True):
        ...
        #new add
        print('save json file before')
        j_outputfile = data.to_json()
        print('save json file comp')
        
        print("----------EXCEL FILE", outputfile, time_stamp)
        after_process_data = time.time()
        print("----------after_save process csv(2)", time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(after_process_data)))

        
    #*** read from csv for reconcil ***    
    timefile, time_concile_stamp = Save_csv(df, '_stock_data.csv')
    
    after_save_csv = time.time()
    time.sleep(60)
    
    print("----------after_save reconcil_csv(3)", time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(after_save_csv)))
    """
    time.sleep(5)
    before_sleep_time = time.time()
    print("----------after_sleep:(2a)", time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(before_sleep_time)))
    starttime = time.time()
    needtime = time.sleep(60.0 - ((time.time() - starttime) % 60.0))
    after_sleep_time = time.time()
    print("----------after_sleep:(2b)", time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(after_sleep_time)))
    """
    return data, j_outputfile
