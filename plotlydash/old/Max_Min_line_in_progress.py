...
def maxminline_strategy(data):
    ...
    ####### BUY part 1#######
    Buyt = np.where((df['Close'] == df['min_close_val'] ) &
                    (df['macdh_vol_gt1'] == 'y') &
                    (slopes20 > 0) &
    ...
    Sellt = np.where((df['Close'] == df['max_close_val']) &
                   (df['macdh_vol_gt1'] == 'y') &
                   (slopes20 > 0) &
                   ((df['macd_above_up'] == 'y') | (df['macd_below_up'] == 'y'))
                     ,df['Close'], "NaN")
    
    df['Buyt_count_tmp'] = np.where(Buyt == 'NaN', 0, 1)
    df['Buyt_count'] = np.add.accumulate(df['Buyt_count_tmp'])
    
    df['Sellt_count_tmp'] = np.where(Sellt == 'NaN', 0, 1)
    df['Sellt_count'] = np.add.accumulate(df['Sellt_count_tmp'])
    
    df['BS_diff'] = df['Buyt_count_tmp'] = df['Buyt_count_tmp']
    df['BS_diff']  = np.where(((df['BS_diff']==1) & (df['BS_diff']==-1)), df['BS_diff'].shift(1) + df['BS_diff'], df['BS_diff'].shift(1)  ) 
    
    df['BS_diff']  = np.where(((df['Buyt_count_tmp']==1) & (df['BS_diff'] .shift() == 0)), +1, df['BS_diff']  ) 
    df['BS_diff'] = np.where(((df['Buyt_count_tmp']==1) & (df['BS_diff'].shift() == -1)), df['BS_diff'], df['BS_diff'] ) 
    df['BS_diff'] = np.where(((df['Buyt_count_tmp']==1) & (df['BS_diff'].shift() == +1)), 0, df['BS_diff'])          
              
    df['BS_diff']  = np.where(((df['Sellt_count_tmp']==1) & (df['BS_diff'].shift() == 0)), -1, df['BS_diff']) 
    df['BS_diff']  = np.where(((df['Sellt_count_tmp']==1) & (df['BS_diff'].shift() == -1)), 0, df['BS_diff']) 
    df['BS_diff'] = np.where(((df['Sellt_count_tmp']==1) & (df['BS_diff'].shift() == +1)), df['BS_diff'], df['BS_diff'] )           



    
    """           
    BS_count =df['Buyt_count_tmp'] - df['Sellt_count_tmp']
    BS_count=np.where(BS_count=='1', 'b', BS_count)
    BS_count=np.where(BS_count=='-1', 's', BS_count)
    BS_count=pd.Series( (v[0] for v in BS_count) )     
    """  
    ########### Re-do the Buy, Sell, Buy_count, Sell_count ##########
    
    Sell = np.where((Sellt != "NaN") & 
                    ((df['Buyt_count'] > df['Sellt_count']) | 
                     (df['Buyt_count'] == df['Sellt_count'])),  
                     Sellt, "NaN")
    
    Buy = np.where((Buyt != "NaN") & 
                   ((df['Sellt_count'] > df['Buyt_count'] ) | 
                   (df['Buyt_count'] == df['Sellt_count'])), 
                    Buyt, "NaN")
    ...
    df['Actual_sell_count'] = np.where(Sell==Sellt, 1, 0)
    
    df['Buyt_count_tmp'] = np.where(Buy == 'NaN', 0, 1)
    df['Buyt_count'] = np.add.accumulate(df['Buyt_count_tmp'])
    
    df['Sell_count_tmp'] = np.where(Sell == 'NaN', 0, 1)
    df['Sell_count'] = np.add.accumulate(df['Sell_count_tmp'])
    

    
    ######## Buy part 2
    #df['Buy_count_compare'] = np.where(Buy == 'NaN', np.nan, 1)
...
def create_dash(flask_app):
        ...
        def update_profitchart(n_intervals, buy, sell):
            if n_intervals == 0:
                ...
                """
                buy_profit = np.where(buy_nplist[0:min_len,1]<sell_nplist[0:min_len,1],
                                     buy_nplist[0:min_len,0] - sell_nplist[0:min_len,0],
                                     sell_nplist[0:min_len,0] - buy_nplist[0:min_len,0])
                """
                for i in range (min_len):
                    ...
                    if(np.minimum(int(buy_nplist[i,1]), int(sell_nplist[i,1])) == int(buy_nplist[i,1])):
                        ...
                        print("buy_profit",i," ",(buy_nplist[i,0]),"<", sell_nplist[i,0],buy_profit)
                    else:
                        long_profit = float(sell_nplist[i,0]) - float(buy_nplist[i,0])
                        print("long_profit_idx",i," ",buy_nplist[i,1],">", (sell_nplist[i,1]), long_profit)
                        print("long_profit",i," ",buy_nplist[i,0],">", (sell_nplist[i,0]), long_profit)
                    #sell -> buy
        ...
        def update_table(n_intervals, buy_nplist, sell_nplist):
            ...
            if n_intervals == 0:
                ...
                profit = np.where(df_c.b_idx.astype(int) > df_c.s_idx.astype(int), 
                                      df_c.sell.astype(float) - df_c.buy.astype(float),
                                      df_c.buy.astype(float) - df_c.sell.astype(float), 
                                       )
                df_c['profit'] = profit
                
                if len(df_s) > len(df_b):
                    styles = [{'if': {'column_id': 'profit', 'filter_query': '{buy} != 0'}, 'color': 'tomato','fontWeight':'normal'},]
                else:
                    styles = [{'if': {'column_id': 'profit', 'filter_query': '{sell} != 0'}, 'color': 'tomato','fontWeight':'normal'},]
