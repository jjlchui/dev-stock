...
def create_dash(flask_app):
        ...
        def update_df(n_intervals):
                if n_intervals == 0:
                    ...
                    filename = "D:\\Development\\flask dev\\stock\\data\\2023-03-03 NQ=F USTime_out_stock_data.csv"
                    cwd = os.getcwd()
                    ...
                    #df = pd.read_csv("D:\\Development\\flask dev\\stock\\data\\test_result.csv", names =['Datetime','Open','High','Low','Close', 'Volume'])
        ...
        def renko(n_intervals, data):
                if n_intervals == 0:
                    ...
                    print("$$$$$$ min/max uptrend", df['uptrend'].iloc[-1], df['uptrend'].iloc[-2] )
                    print("$$$$$$ min/max uptrend", df['uptrend'].iloc[-1] , df['uptrend'].shift().iloc[-1] )
                    print("$$$$$$ min/max close", min_Close[-1], max_Close[-1] )
        ...
        def cal_count(n_intervals, tot_tbuy, tot_tsell, buy_tcount, sell_tcount):
                if n_intervals == 0:
                    ...
                    buy_tcount = buy_tcount.item()
                    sell_tcount = sell_tcount.item()
                    
                    if (buy_tcount >= 0) and (sell_tcount >= 0):
                        ave_b = tot_tbuy / buy_tcount
                        ave_b = np.around(ave_b, 2)
                        buy_count = buy_tcount
                        tot_buy = tot_tbuy

                        ave_s = int(tot_tsell / sell_tcount)
        ...
        def update_profitchart(n_intervals, buy, sell):
            if n_intervals == 0:
                ...
                buy_list = []
                buy_idx = []
                for idx, xs in enumerate(buyitem) or []:
                    if xs != 'NaN':
                        buy_list.append(xs)
                        buy_idx.append(idx)
                    else:
                ...
                return buy_list, buy_idx

                sell_list = []
                sell_idx = []
                for idx, xs in enumerate(sellitem) or []:
                    if xs != 'NaN':
                        sell_list.append(xs)
                        sell_idx.append(idx)
                    else:
                        return
                return sell_list, sell_idx
