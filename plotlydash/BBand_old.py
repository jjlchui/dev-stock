...
def create_dash(flask_app):
        ...
        app=dash.Dash(server=flask_app, name="stock_dash", url_base_pathname=("/bband/"), prevent_initial_callbacks=True)
        ...
        def update_df(n_intervals):
                if n_intervals == 0:
                    ...
                    df.ta.strategy(momo_bands_sma_strategy )
        ...
        def update_graph(n_intervals, data, buy, sell):
            ...
            if n_intervals == 0:
                ...
                figure = go.Figure(
                    data = [
                            go.Scattergl(x=df.index, y=df.Close, line=dict(color='#DE847B', width=1.5),
                            name = 'Close',
                            hoverinfo = 'text',
                            hovertext =
                            ...
                            go.Scattergl(x=df.index, y=df["BBU_20_2.0"] , line=dict(color='#ffcf40', width=2, dash='dot'),
                                name = 'Upper BBU',
                                hoverinfo = 'text',
                                hovertext =
                                '<b>Time</b>: ' + df.Datetime.astype(str) + '<br>' +
                                '<b>Price</b>: ' + [f'{x:,.2f}' for x in df.MA20] + '<br>'),
                            go.Scattergl(x=df.index, y=df["BBM_20_2.0"], line=dict(color='#ffbf00', width=1),
                                name = 'Mid BB',
                                hoverinfo = 'text',
                                hovertext =
                                '<b>Time</b>: ' + df.Datetime.astype(str) + '<br>' +
                                '<b>Price</b>: ' + [f'{x:,.2f}' for x in df.MA50] + '<br>'),
                            go.Scattergl(x=df.index, y=df["BBL_20_2.0"], line=dict(color='#ffcf40', width=2, dash='dot'),
                                name = 'lower BB',
                                hoverinfo = 'text',
                                hovertext =
                                '<b>Time</b>: ' + df.Datetime.astype(str) + '<br>' +
