...
#@scheduler.scheduled_job("cron", hour="0-23", minute = "*", id='my_job_id')
scheduler = BlockingScheduler()
@scheduler.scheduled_job('cron', hour='0-23', minute = '*', day_of_week = '1-5', id="my_job_id")


def get_stock_data():

    if (datetime.datetime.now().strftime('%H:%M:%S')[:-3] == "12:15"):
        start_time = datetime.datetime.now()
        print("*******START TIME(1)", start_time)
        end_time_stamp = start_time + timedelta(minutes=30)
        print("*******END TIME(1)", end_time_stamp)
        Running = True
    else:
        Running = False
        print("not ready")
    ...
    while (Running):    
        ...
        print ("end", end_time_stamp, "current", current_time_stamp)
        if (current_time_stamp > end_time_stamp):
            scheduler.remove_job('my_job_id')
            Running = False 
            print ("*****inside job executing inside", )
            break
    
#sched = BlockingScheduler(timezone='America/New_York')
#job start 9:am to 5:pm
#sched = BlockingScheduler()
#sched.add_job(get_stock_data, 'cron', hour='5-23', minute = '*')
scheduler.start() 
