...
def Save_csv(file, filename):  
    ...
    #file_path = /home/jjhui/stock_app/plotlydash/NDX.py/
    ...
    #file_path = os.path.join(path, "/home/jjhui/stock_app/data/")
...
def get_stock_data():
    ...
    print("inside get_stock_data", time_stamp)
...
print("US Eastern timezone DateTime 1:", dt_us_eastern.strftime("%Y:%m:%d %H:%M:%S %Z %z"))

scheduler = BackgroundScheduler() # Create Scheduler
scheduler.configure(timezone=pytz.timezone('America/New_York'))

#scheduler.add_job(get_stock_data, "interval", minutes=1) # Add job to scheduler
scheduler.add_job(get_stock_data, "cron", hour="0-23", minute = "*")
