...
def create_dash(flask_app):
        ...
        def update_df(n_intervals):
                if n_intervals == 0:
                    ...
                    filename = "2022-11-05 NQ=F USTime_feather_stock_data.feather"
                    cwd = os.getcwd()
        ...
        def maxminline_strategy(n_intervals, data):
                if n_intervals == 0:
                    ...
                    df['Buy_n'] = T_Buy
                    df['Buy_n'] = df['Buy_n'].fillna(0)
                    df['Sell_n'] = T_Buy
                    df['Sell_n'] = df['Sell_n'].fillna(0)
                    
                    df['Sell_n'] = np.where(df['Sell_n'] == 0, 0, 1)
                    df['Buy_n'] = np.where(df['Buy_n'] == 0, 0, 1)
                    df['Buy_n_count'] = np.add.accumulate(df['Buy_n'])
                    df['Sell_n_count'] = np.add.accumulate(df['Sell_n']) 
                    
                    if (df['Buy_n_count'] > df['Sell_n_count']):  
                    ...
                    """                       
                        df['Sell_n_f'] = np.where(#(df['Buy_n'].count() > df['Sell_n'].count()) & 
                                             ((df['Close'] < df['Buy_loss_limit']) &
                                             (df['Close'].shift() > df['Buy_loss_limit']))
                                            , df.Close, T_Sell)
                        #T_Sell = df['Sell_n_f']
                    
                    
                    if (df['Sell_n_count'] - df['Buy_n_count'] == 1):               
                        ...
                        df['Buy_n_f'] = np.where(#(df['Sell_n'].count() > df['Buy_n'].count()) &
                                            ((df['Close']> df['Sell_loss_limit']) & 
                                            (df['Close'].shift() < df['Sell_loss_limit']))
                                            , df.Close, T_Buy)
                        T_Buy = df['Buy_n_f']   
                   
                    """
                    """
                    lst_b_loss_limit = [i for i in df['Buy_loss_limit'] if not math.isnan(i)][-1]
                    tmp_frist_s = [i for i in df['Close'] if i >  lst_b_loss_limit][-1]
                    
                    
                    df['Sell_n_f_new'] = np.where((df['Buy_n'].count() > df['Sell_n'].count()) & 
                                                  (df['Close'] == tmp_frist_s)
                                                  , df.Close, T_Sell)
